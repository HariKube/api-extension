...
func TestCountListHandlerReturnsCountResponseList(t *testing.T) {
	originalSAR := countSubjectAccessReview
	originalGetResource := countGetResource
	originalCountGet := countGet
	t.Cleanup(func() {
	...
	if gotResourceAttributes.Namespace != "default" || gotResourceAttributes.Verb != "list" || gotResourceAttributes.Group != "" || gotResourceAttributes.Resource != "pods" {
